...
思维小岛是一个面向小学阶段的开源数学思维训练项目。它借鉴 Duolingo 式短关卡、即时反馈、连续学习和经验值机制，让孩子在轻量练习里建立数感、规律观察、比较推理、应用建模和空间意识。当前优先服务 1-3 年级，先开放一年级基础路线。
...
- 每关维护扩展题库，每次练习从题库中抽取 5 道低年级友好的题目
- 当前开放一年级基础路线，题库已预留 1-3 年级扩展元数据
- 即时反馈和解题说明
- 错题复习模式，错题需连续答对 2 次才会清除，记录只保存在本机浏览器中
- 本地学习概览，展示总练习次数、已练关卡、当前错题和最近成绩
- 颜色和图形提示会自动渲染成更直观的视觉 token
- 经验值、星星、连续天数
- 本地浏览器保存学习进度
- 适配桌面和手机屏幕，学习路线信息会自动换行避免拥挤
...
题库位于 `data/lessons.js`。每个关卡至少应维护 10 道题，应用每次只抽取 5 道，避免孩子通过记忆固定题序通关。每道题需要包含：
...
每个关卡还需要包含扩展元数据：`grade`、`gradeBand`、`domain` 和 `level`。当前阶段 `gradeBand` 使用 `lower`，表示 1-3 年级优先路线；现有内容全部标记为一年级基础题库。

为了保护 1-3 年级孩子的阅读体验，题干、解释、选项和视觉提示都有长度限制。修改题库后请运行 `npm run check`。
